...
export default function TrainingFeatureBar() {
  return (
    <div className="relative z-10 mt-4 w-full shrink-0 border-t border-white/10 bg-black/45 backdrop-blur-[3px] sm:mt-0 lg:mt-0">
      <div className="grid w-full grid-cols-1 divide-y divide-white/10 sm:grid-cols-2 sm:divide-y-0 lg:grid-cols-4">
        {TRAINING_FEATURES.map(({ title, description }, index) => (
          <div
            key={title}
            className="flex items-start gap-3 px-4 py-3.5 sm:items-center sm:px-5 sm:py-3.5 lg:border-r lg:border-white/10 lg:last:border-r-0"
          >
            <HugeiconsIcon
              icon={icons[index]}
              size={18}
